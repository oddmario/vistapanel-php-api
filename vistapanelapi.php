...
class vistapanelApi {
  ...
  function CheckLogin() {
    ...
    $this->CheckCpanelUrl();
		...
		$login = $this->SimpleCurl($this->cpanel_url . "/login.php", true, array("uname" => $username, "passwd" => $password, "theme" => $theme, "seeesurf" => "567811917014474432"), true, array(), true);
    ...
    if( !empty($cookies['PHPSESSID']) ) {
        if( strpos($login, "document.location.href = 'panel/indexpl.php") !== false ) {
            if( $this->loggedin !== true ) {
                ...
                $checkImportantNotice = $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php", false, array(), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
                if( strpos($checkImportantNotice, "To notify you of changes to service and offers we need permission to send you email") !== false ) {
                    $this->SimpleCurl($this->cpanel_url . "/panel/approve.php", true, array("submit" => true), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
                }
            } else {
  ...
  function createDatabase($dbname = "") {
      ...
      $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=mysql&cmd=create", true, array("db" => $dbname), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
      return true;
  }
  ...
  function listDatabases() {
      ...
      $this->CheckLogin();
      $databases = array();
      $htmlContent = $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=pma", false, array(), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
      $DOM = new DOMDocument();
  ...
  function deleteDatabase($database = "") {
      ...
      $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=mysql&cmd=remove", true, array("toremove" => $this->accountUsername . "_" . $database, "Submit2" => "Remove Database"), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
      return true;
  }
  ...
  function getToken() {
      $this->CheckCpanelUrl();
      $this->CheckLogin();
      $homepage = $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php", false, array(), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
  ...
  function listAddonDomains() {
      ...
      $this->CheckLogin();
      $addonDomains = array();
      $htmlContent = $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=domains&ttt=" . $this->vistapanel_token, false, array(), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));  
      $DOM = new DOMDocument();
      libxml_use_internal_errors(true);
      $DOM->loadHTML($htmlContent);
      libxml_clear_errors();
  ...
  function listSubDomains() {
      ...
      $this->CheckLogin();
      $subDomains = array();
      $htmlContent = $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=subdomains&ttt=" . $this->vistapanel_token, false, array(), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));  
      $DOM = new DOMDocument();
      libxml_use_internal_errors(true);
      $DOM->loadHTML($htmlContent);
      libxml_clear_errors();
  ...
  function SetPhpVersion($domain, $newversion) {
      ...
      if( !in_array($newversion, array("54", "55", "56", "70")) ) {
          $this->classError("Invalid newversion, please read the documentation for the possible new versions.");
      }
      $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=phpselectversion_change", true, array("version" => $newversion, "domain_name" => $domain), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
      return true;
  }
  ...
  function GetSoftaculousLink() {
      $this->CheckCpanelUrl();
      $this->CheckLogin();
      $getlink = $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=installer&ttt=" . $this->vistapanel_token, false, array(), true, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session), true);
  ...
  function listEmailAccounts() {
      ...
      $this->CheckLogin();
      $emailaccounts = array();
      $htmlContent = $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=emailaccounts&ttt=" . $this->vistapanel_token, false, array(), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
      $dom = new DOMDocument;
      libxml_use_internal_errors(true);
      $dom->loadHTML($htmlContent);
      libxml_clear_errors();
  ...
  function createEmailAccount($email = "", $domain = "", $password = "") {
      ...
      $this->SimpleCurl($this->cpanel_url . "/panel/modules-new/emailaccounts/add.php", true, array("email" => $email, "d_name" => $domain, "password" => $password, "B1" => "Add"), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session));
      return true;
  }
  ...
  function Logout() {
      $this->CheckCpanelUrl();
      $this->CheckLogin();
      $this->SimpleCurl($this->cpanel_url . "/panel/indexpl.php?option=signout", false, array(), false, array("Cookie: " . $this->vistapanel_sessionName . "=" . $this->vistapanel_session), true);
      $cpanel_url = "";
      $loggedin = false;
      $vistapanel_session = "";
      $vistapanel_token = 0;
